...
function extractImageInfo($block) {
...
function extractTextInfo($block) {
    $textObject = new stdClass();
...
foreach ($site->layouts()->toBlocks() as $block) {
    if ($block !== null) {
        array_push($apropos_images, extractImageInfo($block));
    }
}

foreach ($site->apropos_text_content()->toBlocks() as $block) {
    if ($block !== null) {
        array_push($apropos_bubles, extractTextInfo($block));
    }
}

foreach ($site->restaurant_text_content()->toBlocks() as $block) {
    if ($block !== null) {
        array_push($restaurant_bubles, extractTextInfo($block));
    }
}

foreach ($site->rest_galerie()->toBlocks() as $block) {
    if ($block !== null) {
        array_push($restaurant_images, extractImageInfo($block));
    }
}

foreach ($site->event_text_content()->toBlocks() as $block) {
    if ($block !== null) {
        array_push($events_bubles, extractTextInfo($block));
    }
}

foreach ($site->evnt_galerie()->toBlocks() as $block) {
    if ($block !== null) {
        array_push($events_images, extractTextInfo($block));
    }
}
...
foreach ($site->contact_text_content()->toBlocks() as $block) {
    if ($block !== null) {
        array_push($contact_bubles, extractImageInfo($block));
    }
}
